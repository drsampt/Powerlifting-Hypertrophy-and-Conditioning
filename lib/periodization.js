...
function buildExercise(name, sets, reps, rpe, profile, opts = {}) {
  ...
  return {
    ...
    warmup_sets: opts.isMain ? generateWarmupSets(weight) : []
  };
}
...
function fourDaySplit(sport, weekIdx, focus, profile) {
  ...
  days.push({
    ...
    exercises: [
      buildExercise(primary1 || 'Back Squat', focus.sets, focus.reps, focus.rpe, profile, { isMain: true }),
      ...
      buildExercise(accessoryPick(lib.accessory, weekIdx + 2), 3, 15, focus.rpe - 1.5, profile)
    ]
  });
  days.push({
    day_name: 'Tuesday',
    ...
    exercises: [
      buildExercise(primary2 || 'Bench Press', focus.sets, focus.reps, focus.rpe, profile, { isMain: true }),
      buildExercise(accessoryPick(lib.accessory, weekIdx + 3), 3, 10, focus.rpe - 1, profile),
      buildExercise(accessoryPick(lib.accessory, weekIdx + 4), 3, 12, focus.rpe - 1, profile),
  ...
  days.push({
    ...
    exercises: [
      buildExercise(primary3 || lib.secondary[0], focus.sets, focus.reps + 1, focus.rpe - 0.5, profile, { isMain: true }),
      ...
      buildExercise(accessoryPick(lib.accessory, weekIdx + 8), 3, 15, focus.rpe - 1.5, profile)
    ]
  });
  days.push({
    day_name: 'Friday',
    ...
    exercises: [
      buildExercise(lib.secondary[1] || primary2, focus.sets, focus.reps + 1, focus.rpe - 0.5, profile, { isMain: true }),
      buildExercise(accessoryPick(lib.accessory, weekIdx + 9), 4, 8, focus.rpe - 1, profile),
      buildExercise(accessoryPick(lib.accessory, weekIdx + 10), 3, 15, focus.rpe - 1.5, profile),
      buildExercise(accessoryPick(lib.accessory, weekIdx + 11), 3, 15, focus.rpe - 1.5, profile)
    ]
  });
  days.push({
    day_name: 'Saturday',
  ...
  return [
    {
      ...
      exercises: [
        buildExercise(meLift, 1, 2, 9, profile, { isMain: true }),
        ...
        buildExercise(accessoryPick(lib.accessory, weekIdx + 2), 3, 15, 6.5, profile)
      ]
    },
    {
      day_name: 'Wednesday',
      ...
      exercises: [
        buildExercise(deLift, 8, 3, 6.5, profile, { isMain: true }),
        buildExercise(accessoryPick(lib.accessory, weekIdx + 3), 4, 10, 7, profile),
        buildExercise(accessoryPick(lib.accessory, weekIdx + 4), 3, 15, 6.5, profile),
        buildExercise(accessoryPick(lib.accessory, weekIdx + 5), 3, 15, 6.5, profile)
      ]
    },
    {
      day_name: 'Friday',
      ...
      exercises: [
        buildExercise(repLift, 4, 10, 7.5, profile, { isMain: true }),
        buildExercise(accessoryPick(lib.accessory, weekIdx + 6), 4, 10, 7.5, profile),
        buildExercise(accessoryPick(lib.accessory, weekIdx + 7), 3, 15, 7, profile),
        buildExercise(accessoryPick(lib.accessory, weekIdx + 8), 3, 15, 7, profile)
      ]
    },
    {
      day_name: 'Saturday',
...
function dupWeek(sport, weekIdx, profile) {
  ...
  return types.map((t, i) => ({
    ...
    workout_type: t.name,
    exercises: [
      buildExercise(lib.primary[i % lib.primary.length], t.sets, t.reps, t.rpe, profile, { isMain: true }),
      buildExercise(accessoryPick(lib.accessory, weekIdx + i * 3), 3, 10, t.rpe - 1, profile),
      buildExercise(accessoryPick(lib.accessory, weekIdx + i * 3 + 1), 3, 12, t.rpe - 1, profile),
      buildExercise(accessoryPick(lib.accessory, weekIdx + i * 3 + 2), 3, 15, t.rpe - 1.5, profile)
    ]
  }));
}
...
function generateConjugate(profile) {
  ...
  for (let w = 1; w <= totalWeeks; w++) {
    ...
    if (isDeload) {
      days = days.map(d => ({ ...d, exercises: d.exercises.map(applyDeloadToExercise) }));
    }
