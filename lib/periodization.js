...
// A 1RM only applies directly to the exact lift it was tested on — a Front Squat max
// is not "your squat max," it's its own number. So the only exercises that get a load
// computed off the profile's squat/bench/deadlift fields are those three lifts by exact
// name. Every other exercise (variants, secondary lifts, sport-specific primaries like
// Snatch or Log Press, accessories) needs its own tested/logged max in
// profile.exercise_maxes (an { "Exercise Name": oneRm } map the server attaches before
// generation) — if the athlete hasn't tested it yet, it's prescribed by RPE instead of a
// guessed weight, same as any other accessory.
function relevantMax(profile, exerciseName) {
  if (exerciseName === 'Back Squat') return profile.squat_max;
  if (exerciseName === 'Bench Press') return profile.bench_max;
  if (exerciseName === 'Deadlift') return profile.deadlift_max;
  const maxes = profile.exercise_maxes || {};
  return maxes[exerciseName] || null;
}
