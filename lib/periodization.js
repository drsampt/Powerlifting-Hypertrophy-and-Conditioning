...
function buildDay(dayName, workoutType, mainSpecs, muscleFocus, lib, profile, weekVolume, durationRange, volumeMultiplier, weekIdx, weekUsedNames) {
  ...
  // weekUsedNames (shared across every day in the week) keeps the same accessory from
  // showing up twice in one week — different days should train different exercises.
...
function fourDaySplit(sport, weekIdx, focus, profile, durationRange) {
  ...
  const mondayMain = primary1 || 'Back Squat';
  const tuesdayMain = primary2 || 'Bench Press';
  const thursdayMain = primary3 || lib.secondary[0];
  const fridayMain = lib.secondary[1] || primary2;

  const weekVolume = {};
  // Reserve every day's main lift before any accessory picking starts — otherwise an
  // earlier day's accessory loop could grab a lift another day is about to use as its main.
  const weekUsedNames = new Set([mondayMain, tuesdayMain, thursdayMain, fridayMain]);
  const volumeMultiplier = phaseVolumeMultiplier(focus.reps);
  const days = [];

  days.push(buildDay('Monday', 'Lower A',
    [{ name: mondayMain, sets: focus.sets, reps: focus.reps, rpe: focus.rpe }],
    [...LOWER_MUSCLES, 'core'], lib, profile, weekVolume, durationRange, volumeMultiplier, weekIdx, weekUsedNames));

  days.push(buildDay('Tuesday', 'Upper A',
    [{ name: tuesdayMain, sets: focus.sets, reps: focus.reps, rpe: focus.rpe }],
    [...UPPER_MUSCLES, 'core'], lib, profile, weekVolume, durationRange, volumeMultiplier, weekIdx + 1, weekUsedNames));

  days.push(buildDay('Thursday', 'Lower B',
    [{ name: thursdayMain, sets: focus.sets, reps: focus.reps + 1, rpe: focus.rpe - 0.5 }],
    [...LOWER_MUSCLES, 'core'], lib, profile, weekVolume, durationRange, volumeMultiplier, weekIdx + 2, weekUsedNames));

  days.push(buildDay('Friday', 'Upper B',
    [{ name: fridayMain, sets: focus.sets, reps: focus.reps + 1, rpe: focus.rpe - 0.5 }],
    [...UPPER_MUSCLES, 'core'], lib, profile, weekVolume, durationRange, volumeMultiplier, weekIdx + 3, weekUsedNames));
...
function taperExercise(e, weeksOut) {
  if (!e.is_main && e.weight_percentage == null) return null; // drop pure accessory/isolation work
  if (!e.is_main) return null; // secondary compounds also drop out during taper for max specificity
...
function taperDay(day, weeksOut) {
  const exercises = day.exercises
    .map(e => taperExercise(e, weeksOut + liftTaperOffset(e.name)))
    .filter(Boolean);
...
function applyPeaking(phases, testingWeek, experienceLevel, deloadWeeks) {
  ...
  for (const phase of phases) {
    for (const workout of phase.workouts) {
      if (workout.week === overreachWeek && overreachWeek >= 1) {
        ...
        workout.days = workout.days.map(d => taperDay(d, weeksOut));
        workout.taper = true;
...
function finalizeProgram(profile, phases, deloadWeeks, periodizationType) {
  ...
  const peaking = applyPeaking(phases, testingWeek, profile.experience_level, deloadWeeks);
