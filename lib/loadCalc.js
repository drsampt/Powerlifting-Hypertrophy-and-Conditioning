...
// Ascending warm-up ramp leading to the working weight (standard 40/55/70/85% progression).
// Each step is dropped if it rounds to >= the working weight or to 0.
const WARMUP_SCHEME = [
  { pct: 0.4, reps: 5 },
  { pct: 0.55, reps: 3 },
  { pct: 0.7, reps: 2 },
...
function generateWarmupSets(workingWeight, increment = 5) {
  if (!workingWeight || workingWeight <= 0) return [];
  const sets = [];
  for (const step of WARMUP_SCHEME) {
    const weight = roundToIncrement(workingWeight * step.pct, increment);
    if (weight > 0 && weight < workingWeight) {
      sets.push({ reps: step.reps, weight });
    }
  }
  return sets;
}

module.exports = { percentForRepsRpe, calcWeight, roundToIncrement, rirFromRpe, epley1RM, nearestHalf, generateWarmupSets };
